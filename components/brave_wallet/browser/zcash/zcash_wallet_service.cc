...
void ZCashWalletService::IsAccountShielded(mojom::AccountIdPtr account_id,
                                           IsAccountShieldedCallback callback) {
#if BUILDFLAG(ENABLE_ORCHARD)
  if (IsZCashShieldedTransactionsEnabled()) {
    auto birthday = GetAccountShieldBirthday(account_id);
    std::move(callback).Run(!birthday.is_null());
    return;
  }
#endif
...
void ZCashWalletService::GetShieldSyncStatus(
    mojom::AccountIdPtr account_id,
    GetShieldSyncStatusCallback callback) {
#if BUILDFLAG(ENABLE_ORCHARD)
  if (IsZCashShieldedTransactionsEnabled()) {
...
void ZCashWalletService::GetAccountShieldBirthday(
    mojom::AccountIdPtr account_id,
    GetAccountShieldBirthdayCallback callback) {
  auto birthday = GetAccountShieldBirthday(account_id);
  if (!birthday) {
